// Mobile capture pass — headless Chromium at 390x844 with the session cookie.
...
const OUT_DIR = path.join(import.meta.dirname, "..", "evidence", "shots", "mobile");
...
for (const page of PAGES) {
  ...
  r.lightProbe = await pg.evaluate(PROBE);  await pg.screenshot({ path: path.join(OUT_DIR, page.slice(1) + "-mobile-light.png"), fullPage: true });
  ...
  await pg.screenshot({ path: path.join(OUT_DIR, page.slice(1) + "-mobile-dark.png"), fullPage: true });
  await pg.close();
  results[page] = r;
...
console.log("BYTE_LIGHT=" + fs.statSync(path.join(OUT_DIR, "accounts-mobile-light.png")).size);
console.log("BYTE_DARK=" + fs.statSync(path.join(OUT_DIR, "accounts-mobile-dark.png")).size);
await browser.close();
console.log("DONE");